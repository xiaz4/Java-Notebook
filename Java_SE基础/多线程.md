...
wait 方法：是 Object 的方法，必须与 synchronized 关键字一起使用，线程进入阻塞状态进入对象的等待池（wait pool），睡眠时，释放互斥锁。当其他线程 notify 或者 notifyAll 被调用后，会解除阻塞，线程进入等锁池（lock pool）。一旦一个线程获取对象锁，并且从他上次调用wait()方法后的位置开始执行。
